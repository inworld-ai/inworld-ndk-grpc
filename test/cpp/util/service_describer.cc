...
std::string DescribeServiceList(std::vector<std::string> service_list,
                                grpc::protobuf_inworld::DescriptorPool& desc_pool) {
  std::stringstream result;
  for (auto it = service_list.begin(); it != service_list.end(); it++) {
    auto const& service = *it;
    const grpc::protobuf_inworld::ServiceDescriptor* service_desc =
...
std::string DescribeService(const grpc::protobuf_inworld::ServiceDescriptor* service) {
  std::string result;
...
std::string DescribeMethod(const grpc::protobuf_inworld::MethodDescriptor* method) {
  std::stringstream result;
...
std::string SummarizeService(const grpc::protobuf_inworld::ServiceDescriptor* service) {
  std::string result;
  for (int i = 0; i < service->method_count(); ++i) {
...
std::string SummarizeMethod(const grpc::protobuf_inworld::MethodDescriptor* method) {
